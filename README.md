...
**Change the header photo:** save your new photo over `media/images/hero.jpeg`, then run `python3 tools/build-images.py` (needs Python 3 and Pillow — `pip install Pillow`). It rewrites the nine small `hero-600/900/1200.avif/.webp/.jpg` files that the page actually serves, so visitors download ~42 KB instead of the multi-megabyte original. Commit everything it writes. If the new photo is a different shape the script prints the `width`/`height` numbers to put on the `<img>` tag in `index.html`.
